...
function generatedSchedules(index, examType = "written") {
    const firstMonth = (index % 6) + 1;
    const secondMonth = firstMonth + 6;
    const pad = (value) => String(value).padStart(2, "0");
    const resultDate = (month) => month === 12 ? "2027-01-10" : `2026-${pad(month + 1)}-10`;
    return [
        {
...
certifications.push(...additionalCertifications.map((certification, index) => ({
    ...certification,
    schedules: generatedSchedules(index, certification.type === "국제자격" || certification.id.includes("photoshop") ? "practical" : "written")
})));
